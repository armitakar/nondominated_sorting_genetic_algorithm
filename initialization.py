...
def initialization(road_dat, nn, popsize, grp, len_min, len_max): 
    ...
    # estimating crowd-continuous index
...
def generate_naive_solutions(road_dat, nn, min_len):
    rd_id = list(road_dat['OBJECTID'].values)
    rd_len = list(road_dat['length'].values)
    ...
    tot_len = calculate_len(rd_len, sol)
    if (tot_len < min_len) or (tot_len > (min_len + 1000)):
        sol = [0 for i in range(len(road_dat))]
        sol = generate_naive_solutions(road_dat, nn, min_len)
    else:
        print(tot_len)
    return sol
